...
import type { BoardTask, Status } from './types'
...
export function BoardColumn({ title, tasks, status, contributors }: BoardColumnProps) {
    ...
    return (
        <div className="flex-1 min-w-70 max-w-[320px] flex flex-col bg-muted/30 rounded-lg p-2 h-full">
            ...
            <div ref={setNodeRef} className="flex-1 overflow-y-auto px-1 scrollbar-thin rounded-md">
                <SortableContext items={taskIds} strategy={verticalListSortingStrategy}>
                    {tasks.map(task => (
                        <TaskCard key={task.id} task={task} contributors={contributors} />
                    ))}
