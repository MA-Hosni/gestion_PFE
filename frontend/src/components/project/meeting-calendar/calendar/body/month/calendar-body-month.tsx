...
export default function CalendarBodyMonth() {
  const { date, meetings, setDate, setMode } = useCalendarContext()
  ...
  // Filter meetings to only show those within the current month view
  const visibleMeetings = meetings.filter((meeting) =>
    isWithinInterval(meeting.scheduledDate, {
      start: calendarStart,
      end: calendarEnd,
    })
  )
        ...
        >
          {calendarDays.map((day) => {
            const dayMeetings = visibleMeetings.filter((meeting) =>
              isSameDay(meeting.scheduledDate, day)
            )
